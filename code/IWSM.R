...
makeIWSMPlots = function(logisticApproximation=FALSE, signif=.95) {
  ...
  rangePrevalenceCVPixel = c()
  rangePrevalenceCVConstituency = c()
  rangePrevalenceCVCounty = c()
  rangePrevalenceCVProvince = c()
  ...
  rangeCountCVPixel = c()
  rangeCountCVConstituency = c()
  rangeCountCVCounty = c()
  rangeCountCVProvince = c()
  ...
  rangeRelativePrevalenceCVConstituency = c()
  rangeRelativePrevalenceCVCounty = c()
  rangeRelativePrevalenceCVProvince = c()
  ...
  for(i in 1:length(areaLevels)) {
    ...
    if(thisLevel == "pixel") {
      ...
      prevalenceCVConstituency = prevalenceSDConstituency / rowMeans(subareaPop$pSmoothRisk)
      rangePrevalenceCVConstituency = range(prevalenceCVConstituency)
      ...
      browser()
      prevalenceCVConstituencylcpb = prevalenceSDConstituencylcpb / rowMeans(subareaPop$pSmoothRisk)
  ...
  logit1000 = function(x) {logit(x/1000)}
  expit1000 = function(x) {expit(x)*1000}
  ...
  pdf(paste0(figDirectory, "IWSM/paige-prevalenceMeanCIWidth", logisticText, ".pdf"), width=10, height=5)
  par(mfrow=c(1,2), mar=c(3, 3.0, 0, 3.1), oma=c(0, 0, 0, 0.7), mgp=c(1.9,.7,0))
  
  meanTicks = pretty(meanRange*1000, n=5)
  meanTickLabels = as.character(meanTicks)
  
  # constituency level
  plotMapDat(plotVar=1000*constituencyMean, mapDat=constituencyMap, new = TRUE, 
             main="", scaleFun=logit1000, scaleFunInverse=expit1000, 
             cols=meanCols, zlim=logit1000(1000*meanRange), ticks=meanTicks, tickLabels=meanTickLabels, 
             xlim=thisKenyaLonRange, ylim=thisKenyaLatRange, addColorBar = TRUE, 
  ...
  widthTicksSubarea = pretty(rangePrevalenceCVConstituency, n=8)
  widthTickLabelsSubarea = as.character(widthTicksSubarea)
  widthRangeSubarea = rangePrevalenceCVConstituency
  plotMapDat(plotVar=prevalenceCVConstituency, mapDat=constituencyMap, new = TRUE, 
             main="", scaleFun=log, scaleFunInverse=exp, 
             cols=sdCols, ticks=widthTicksSubarea, tickLabels=widthTickLabelsSubarea, 
             zlim=log(widthRangeSubarea), xlim=thisKenyaLonRange, ylim=thisKenyaLatRange, addColorBar = TRUE, 
             legendArgs=list(axis.args=list(cex.axis=1, tck=-.7, hadj=.1), legend.cex=2, smallplot=c(.88,.91,.2,.9)), legend.width=3, 
             plotArgs=list(cex.main=1, cex.axis=1, cex.lab=1), legend.mar=0, lwd=.1, border=rgb(.4,.4,.4, .7), 
             xlab="Longitude", ylab="")
  plotMapDat(mapDat=countyMap, lwd=.5, border=rgb(.4,.4,.4))
...
plotFig1 = function() {
  ...
  # 95% CI Width ----
  pdf(paste0("figures/IWSM/paige-CIWidthVRes95.pdf"), width=5, height=5)
  ...
  pdf(paste0(figDirectory, "IWSM/paige-EAsPerCon.pdf"), width=5, height=5)
...
makeTable1 = function() {
  ...
  row.names(tab) = rep(c("Empirical", "Gridded"), 2)
