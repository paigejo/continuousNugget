...
popGridWajirNorth = popGrid[popGrid$admin2=="Wajir North",]
popGridWajirAdjusted = popGridAdjusted[popGridAdjusted$admin1=="Wajir",]
poppcSimpleAdjusted = poppc
poppcSimpleAdjusted$popUrb = easpc$EAUrb * 25
poppcSimpleAdjusted$popRur = easpc$EARur * 25
poppcSimpleAdjusted$popTotal = easpc$EATotal * 25
popMatSimpleAdjusted = adjustPopGrid(popGrid, poppcSimpleAdjusted)
...
poppconW = poppcon
poppconW = poppconW[poppconW$County == "Wajir", ]
...
if(FALSE) {
  ...
  popGridFineAdjusted = adjustPopGrid(popGridFine, poppcSimpleAdjusted)
  ...
  popRangeTarget = range(popGridFineAdjusted$popOrig)
  ...
  image.plot(zlim=log(popRangeTarget), nlevel=length(popCols), legend.only=TRUE, horizontal=FALSE,
             col=popCols, add=TRUE, axis.args=list(at=log(popTicks), labels=popTickLabels, cex.axis=1, tck=-.7, hadj=-.1),
             legend.cex=.5, legend.width=1)
  quilt.plot(popGridFineAdjusted$lon, popGridFineAdjusted$lat, log(popGridFineAdjusted$popOrig),
             col=popCols, nx=230, ny=380, add.legend = FALSE, add=TRUE,
...
popWeights = popGridWajirAdjusted$popOrig / sum(popGridWajirAdjusted$popOrig)
...
popWeights = popMatSimpleAdjustedNW$popOrig / sum(popMatSimpleAdjustedNW$popOrig)
...
wajirCountyI = sort(unique(poppc$County)) == "Wajir"
...
out = aggregate(popMatSimpleAdjustedW$popOrig, by=list(pixels=popMatSimpleAdjustedW$admin2), FUN=length)
nPixels = out$x
...
# offsets[6,1] = .15 # shift label for Wajir West slightly farther east
easpcon = meanEAsPerCon()
easpcon = easpcon[easpcon$County=="Nairobi",]
...
testSamples = simDatMulti$aggregatedResultsLcpb$constituencyMatrices$p[poppcon$County=="Wajir",]
testCIWidths = apply(testSamples, 1, function(x){diff(quantile(x, probs=c(0.1, 0.9), na.rm=TRUE))})

sum(popMatSimpleAdjusted$popOrig[popMatSimpleAdjusted$constituency == "Wajir South" & popMatSimpleAdjusted$urban])

temp = popMatSimpleAdjusted
temp$admin2 = as.character(temp$admin2)
temp = temp[temp$admin1 == "Wajir",]
aggregate(temp$popOrig, by=list(admin2=temp$admin2, urban=temp$urban), FUN=sum, drop=FALSE)
#         admin2 urban         x
